 ...
 * @author tcolburn
 */
public class PuzzleBigState extends State {
     ...
     */
    public PuzzleBigState(int[][] tiles) {
        this.tiles = tiles;
    }
     ...
     * for this state
     */
    public PuzzleBigState (PuzzleBigState state, Location loc1, Location loc2) {
    ...
    public boolean equals(Object o) {
        if (o == null) return false;
        if (o.getClass() != PuzzleBigState.class) return false;
        PuzzleBigState other = (PuzzleBigState) o;
...
//        return sumManhattan(this, goal);
    ...
    public static int tilesOutOfPlace(State current, State goal) {
        int count = 0;
        int[][] curTiles = ((PuzzleBigState)current).getTiles();
        int[][] goalTiles = ((PuzzleBigState)goal).getTiles();
        for (int i=0; i<4; i++) {
            for (int j=0; j<4; j++) {
                if (goalTiles[i][j] != 0 && curTiles[i][j] != goalTiles[i][j]) {
                    count++;
                }
            }
        }
        ...
        for (int i=0; i<4; i++) {
            for (int j=0; j<4; j++) {
                if (goalTiles[i][j] != 0 && curTiles[i][j] != goalTiles[i][j]) {
                    ...
                    Location g = ((PuzzleBigState)goal).getLocation(curTiles[i][j]);
                    sum += Math.abs(c.getColumn()-g.getColumn()) + Math.abs(c.getRow()-g.getRow());
                }
            }
        }
        return sum;
